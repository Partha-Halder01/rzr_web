        ...
        <main class="main-content flex-1 lg:ml-64 min-h-screen">
            ...
            <header class="bg-white border-b border-gray-200 px-4 lg:px-8 py-4 sticky top-0 z-30">
                <div class="flex items-center justify-between gap-4">
                    <!-- Mobile Menu Button -->
                    <button onclick="toggleSidebar()" class="lg:hidden p-2 -ml-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-lg">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="flex-1 min-w-0">
                        <h1 class="text-xl lg:text-2xl font-bold text-gray-900 truncate">{{ $header ?? 'Dashboard' }}</h1>
                    </div>
                    
                    <div class="flex items-center gap-2 lg:gap-4">
                        <span class="text-sm text-gray-500 hidden sm:block">{{ Auth::user()->name }}</span>
                        <div class="w-8 h-8 lg:w-10 lg:h-10 bg-red-100 rounded-full flex items-center justify-center flex-shrink-0">
                            <span class="text-red-600 font-semibold text-sm lg:text-base">{{ substr(Auth::user()->name, 0, 1) }}</span>
                        </div>
                    </div>
                </div>
            </header>
