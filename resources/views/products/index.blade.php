        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @php
                ...
                $iconColors = [
                    ...
                    'sparkles' => 'bg-orange-500',
                    'adjustments' => 'bg-teal-500',
                ...
                // Default icon
                $defaultIcon = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>';
            @endphp
            
            <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                @foreach($categories as $category)
                <a href="{{ route('products.category', $category) }}" class="category-card group">
                    <div class="category-icon {{ $iconColors[$category->icon] ?? 'bg-red-500' }}">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            {!! $iconPaths[$category->icon] ?? $defaultIcon !!}
                        </svg>
                    </div>
