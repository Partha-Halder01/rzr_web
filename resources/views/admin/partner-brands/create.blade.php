<x-layouts.admin>
    @php $header = 'Add Partner Brand'; @endphp
    
    <div class="max-w-2xl">
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
            <div class="p-4 lg:p-6 border-b border-gray-100">
                <h2 class="text-lg font-semibold text-gray-900">Brand Details</h2>
                <p class="text-sm text-gray-500 mt-1">Add a new partner brand logo to display on the home page.</p>
            </div>
            
            <form action="{{ route('admin.partner-brands.store') }}" method="POST" enctype="multipart/form-data" class="p-4 lg:p-6 space-y-5">
                @csrf
                ...
                <div>
                    <label for="logo" class="block text-sm font-medium text-gray-700 mb-2">Brand Logo (Optional)</label>
                    ...
                    <p class="mt-1 text-xs text-gray-500">Upload PNG, JPG, SVG or WebP. Max 2MB. If no logo, brand name will be displayed as styled text.</p>
                    @error('logo')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="color" id="color" name="color" value="{{ old('color', '#4B5563') }}"
                            class="w-12 h-12 rounded-lg border border-gray-200 cursor-pointer">
                        <input type="text" id="color_text" value="{{ old('color', '#4B5563') }}"
                            class="admin-input flex-1" placeholder="#4B5563" onchange="document.getElementById('color').value = this.value">
                    </div>
                    <p class="mt-1 text-xs text-gray-500">Used for text display when no logo is uploaded.</p>
                    @error('color')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
